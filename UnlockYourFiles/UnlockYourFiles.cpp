...
void sub_4010C0(const CHAR* lpBuffer)
{
    DWORD LastError; // eax
    DWORD v2; // eax
    DWORD v3; // eax
...
BOOL sub_401370(CHAR Buffer[9]) {
    ...
    if (!SetCurrentDirectoryA(PathName))
        sub_4010C0("SetCurrentDirectory(\"Files\")");
    hFindFile = FindFirstFileA(FileName, &FindFileData);
    if (hFindFile == INVALID_HANDLE_VALUE)
        sub_4010C0("FindFirstFile");
    do
    {
        v4 = sub_401030(v3, FindFileData.cFileName);
        FindFileData.cAlternateFileName[v4 + 6] = 0;
        sub_401220(FindFileData.cFileName, v3, Buffer);
        ++v6;
    } while (FindNextFileA(hFindFile, &FindFileData));
    if (GetLastError() != ERROR_NO_MORE_FILES)
        sub_4010C0("FindNextFile");
    sub_401160(v6);
    return v6 != 0;
}
...
int main()
{
    ...
    ReadConsoleA(hConsoleInput, Buffer, 8, &NumberOfCharsRead, NULL);
